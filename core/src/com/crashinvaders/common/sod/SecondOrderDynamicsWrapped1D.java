...
/**
 * Same as {@link SecondOrderDynamics1D} but the value is properly wrapped.
 * Very useful for 360 degree/2PI radian rotation.
 */
// A pure gold inspired by https://youtu.be/KPoeNZZ6H4s
public class SecondOrderDynamicsWrapped1D
{
    private final float wrapAround;
    ...
    public SecondOrderDynamicsWrapped1D(float wrapAround) {
    ...
    public void update(float deltaTime, float dst) {
        ...
        float halfWrapAround = wrapAround * 0.5f;
        while (pDst - dst <= -halfWrapAround) {
            dst -= wrapAround;
        }
        while (pDst - dst >= halfWrapAround) {
            dst += wrapAround;
